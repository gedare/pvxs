...
struct GPROp : public OperationBase
{
    ...
    virtual void reExec(const Value& arg) override final
    {
        if(op!=RPC && arg)
            throw std::invalid_argument("Only RPC may reExec() with Value");

        auto context = chan->context;
        context->tcp_loop.call([this, &arg](){
            if(autoExec)
                throw std::logic_error("reExec() requires Operation creation with .autoExec(false)");
            if(state!=Idle)
                return;
